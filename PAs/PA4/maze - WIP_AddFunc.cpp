...
void explore_cell(Location, int, int, int, int, bool**, Queue*);
...
//void explore_cell()
void explore_cell(Location currentLocation, int shiftRow , int shiftCol, char** maze, int rows, int cols, bool** visited, Location** predecessor, Queue *searchQueue)
//void explore_cell(Location currentLocation.row, Location currentLocation.col, int rows, int cols, bool** visited)
{
  currentLocation.row += shiftRow;
  currentLocation.col += shiftCol;
  // explore cell only if it's within the array bounds
  if ( (currentLocation.row < rows) && (currentLocation.row >= 0) && (currentLocation.col < cols) && (currentLocation.col >= 0) )
  {
    // only explore if cell is open
    if ( (maze[currentLocation.row][currentLocation.col] != '#') && (!visited[currentLocation.row][currentLocation.col]) )
    {
      Location newNeighbour;
      newNeighbour.row = currentLocation.row;
      newNeighbour.col = currentLocation.col;
      searchQueue->add_to_back(newNeighbour);

      predecessor[newNeighbour.row][newNeighbour.col].row = newNeighbour.row;
      predecessor[newNeighbour.row][newNeighbour.col].col = newNeighbour.col;
    }
  }
}
...
//void explore_cell(Location currentLocation, int shiftRow , int shiftCol, char** maze, int rows, int cols, bool** visited, Location** predecessor, Queue *searchQueue)
      ...
      explore_cell(currentLocation, -1, 0, maze, rows, cols, visited, predecessor, searchptr);
      explore_cell(currentLocation, 0, -1, maze, rows, cols, visited, predecessor, searchptr);
      explore_cell(currentLocation, 1, 0, maze, rows, cols, visited, predecessor, searchptr);
      explore_cell(currentLocation, 0, 1, maze, rows, cols, visited, predecessor, searchptr); 
