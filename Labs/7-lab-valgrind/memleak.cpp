...
{
  int sum = 0;
  for(int i=0; i < len; i++){   // deleted =. array was overloaded.
    sum += data[i];
  }

  delete [] data; // add this to delete data
  return sum;
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    //delete [] myword;   myword was never allocated
    delete [] otherword;
  }
  else {
...
/*test1
    deleted the allocated array
    deleted the = sign that was accessing memory beyond the end of the array.
  test2
    +1 in otherword was overunning the array.
    new myword was never allocated.
*/
