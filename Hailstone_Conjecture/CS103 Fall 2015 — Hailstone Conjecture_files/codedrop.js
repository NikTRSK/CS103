...
<table><tbody> \n\
<tr class='grade showgrade-only' style='text-align:center' colspan='99'><td>Grade info here.</td></tr> \n\
...
<table class='gradetable grader-only su-only'><tbody> \n\
<tr class='su-only grader-only'><td colspan=99><span class='button enter-grade'>(Super-User Only) Submit Grade</span></tr> \n\
</tbody></table> \n\
<div class='status'></div> " + this.fs_markup 
    ...
    if (this.state.kind == "Lab") {
       ...
       if (this.state.blurbs.lab_preamble)
          ...
          $(this.div).find(".preamble .blurb").html("You must demonstrate this lab <b>in person</b> in your registered lab section <i>or</i> at a TA or CP's office hours. <i>You will get 0 points if you don't demo it in person.</i> You are allowed to wait until check-in is complete before uploading your final version. You can re-upload as many times as you like.");
    }
    ...
    if (this.state.maxscore > 0)
       $(this.div).addClass('showgrade');
 
    if (this.state.rubric) {
       $(this.div).addClass('grader');
       ...
       for (var i=0; i<this.state.rubric.length; i++) {
          ...
          for (var j=0; j<=this.state.rubric[i].points; j++) {
             if (j>0 && j%5 == 0) options += '<br>';
             options += "<span class='box'><input class='points"+j+"' type='radio'"+(j==0?" checked ":"")
             +" name='"+slug+"' value='"+j+"'>"+j+"</span>";
          }
          var ecinfo = this.state.rubric[i].extra_credit?"<i>(Extra credit)</i> ":"";
          row.insertCell(0).innerHTML = ecinfo + this.state.rubric[i].desc;
          row.insertCell(1).innerHTML = ecinfo + options;
       }
       t.insertRow(-1).innerHTML = '<tr><td>Optional Notes (visible to course staff only)<td><textarea class="remarks"></textarea></tr>';
    }
...
AssignmentCodedrop.prototype.getGradeObject = function() {
   ...
   for (var i=0; i<this.state.rubric.length; i++) {
      ...
      if (this.state.rubric[i].extra_credit)
         ectotal += val;
      else
         total += val;
   }
...
AssignmentCodedrop.prototype.setGradeObject = function(obj) {
   ...
   var gradeInfo = "<i>Grade</i>: <b>";
   gradeInfo += obj.total + "/"+this.state.maxscore+"</b>";
   if (this.state.ecmaxscore > 0)
      gradeInfo += " + " + obj.ectotal +"/"+this.state.ecmaxscore+" extra credit";
   gradeInfo += " (entered at " + obj.time + " by <tt>" + obj.grader+"</tt>)";
   ...
   if (this.state.kind == "Assignment") {
         gradeInfo += " <br><i>Note: because rubrics evolve over time, programming assignment grades are tentative until your work is returned to you.</i>";
   }
